...
<?php endforeach;?>
	...
	<div class="form-group">
				...
				<div class="col-sm-9 ">
					<label class="radio-inline">  <input type="radio" class="" name="is_on"  id="is_on1" value="是" <?php if(isset($data_info['is_on'])&&(trim($data_info['is_on'])=='是')) { ?> checked="checked" <?php } ?>            > 是</label><label class="radio-inline">  <input type="radio"  class="" name="is_on"  id="is_on2" value="否"<?php if(isset($data_info['is_on'])&&(trim($data_info['is_on'])=='否')) { ?> checked="checked" <?php } ?>            > 否</label>
				</div>
			</div>
											</fieldset>
